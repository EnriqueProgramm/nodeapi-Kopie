    ...
    User.findOne({email}, (err, user) => {
         ...
         if(err || !user) {
             return res.status(401).json({
                 error: "User with that email not found, please signup first."
             })
         }        
     

    //if the user is found, then user authenticate
...
exports.signout = (req, res) => {
    res.clearCookie("t");
    return res.json({message: "signout completed!"});
}

exports.requireSignin = expressJwt({
    // if the token is valid, express jwt appends the verified users id in an auth key to requrest object
